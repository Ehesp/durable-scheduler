...
export type Schedule = CronSchedule | DelayedSchedule | ScheduledSchedule | NoSchedule;
...
export abstract class DurableScheduler<Env> extends DurableObject<Env> {
  ...
  private async scheduleNextAlarm() {
    ...
    const [nextTask] = await this.db.select({
      date: tasks.date,
    }).from(tasks).where(
      and(
        gt(tasks.date, new Date()),
        ne(tasks.type, 'no-schedule')
      )
    ).orderBy(asc(tasks.date)).limit(1);
    
    if (!nextTask || !nextTask.date) return;
    await this.ctx.storage.setAlarm(nextTask.date);  
  }
  ...
  async alarm(): Promise<void> {
    ...
    const expired = await this.db.select().from(tasks).where(
      and(
        lte(tasks.date, new Date()),
        ne(tasks.type, 'no-schedule')
      )
    );

    const results = await Promise.allSettled(expired.map(async (task) => {
    ...
    if (results.filter(r => r.status === 'rejected').length > 0) {
      console.error('Failed to execute ${failed.length} tasks');
    }

    // If any tasks failed calling `onSchedule`, log them out here.
    const failed = results.filter(r => r.status === 'fulfilled').map(r => r.value).filter(r => r.error);
    if (failed.length > 0) {
  ...
  public async createTask(schedule: Schedule): Promise<string> {
    ...
    if (schedule.type === 'cron') {
      [record] = await this.db.insert(tasks).values({
        id,
  ...
  async countTasks(query?: {
    ...
    return this.db.select({ count: count() }).from(tasks).where(query?.type ? eq(tasks.type, query.type) : undefined).then(r => r[0].count);
  }
  ...
  // Returns a boolean value if the scheduled task existed.
  async updateTask(id: string, task: Omit<Task, 'id' | 'createdAt'>): Promise<boolean> {
    const updated = await this.db.update(tasks).set(task).where(eq(tasks.id, id)).returning();
    await this.scheduleNextAlarm();
  ...
  async queryTasks(query?: QueryOptions): Promise<Task[]> {
    return this.db.query.tasks.findMany({
      ...
      offset: query?.offset,
      orderBy: (tasks, { asc, desc }) => [query?.orderBy === 'asc' ? asc(tasks.date) : desc(tasks.date)],
      where: (tasks, { eq }) => query?.type ? eq(tasks.type, query.type) : undefined,
    });
  }
